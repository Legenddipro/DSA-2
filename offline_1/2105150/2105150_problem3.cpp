...
//to find subset
 ...
 //for filtering subset by V-1 and weight  = cost
  ...
  clear();
   ...
   vector<int> pseudo_edge;
   // for(auto i:set2)
   for(int i=0;i<set2.size();i++)
    {
       // cout<<*i.mst_count<<endl;
    
     if((*(set2[i].mst_count)) == 0)
     {
        //cout<<"None"<<endl;
        continue;
     }
    else if((*(set2[i].mst_count)) == h)
     {
       // cout<<"any"<<endl;
       critical_edge.push_back(i);
     }
     else{
        //cout<<"at least"<<endl;
        pseudo_edge.push_back(i);
     }
    }
    cout<<"critical_edge ";
    cout<<"[ ";
    for(auto i: critical_edge)
    {
        cout<<i<<",";
    }
     cout<<"] ";
