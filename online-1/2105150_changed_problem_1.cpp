...
int main()
{
    ...
    */
   int no_house,no_pipe;
   cin>>no_house>>no_pipe;
    int no_vertices,no_edges,cost=0;
    no_vertices = no_house +1;
    no_edges = no_pipe+no_house;   // cin>>no_vertices>>no_edges;
    ...
    for(int i =0;i<no_house;i++)
    {
    Edges1 e(0,i+1,weights[i]);
        set2.push_back(e);
      edges.push_back({weights[i],0,i+1});
    }
...
//to find subset
 ...
 //for filtering subset by V-1 and weight  = cost
  ...
  clear();
    ...
    //TO PRINT ANS
    int h =1;
    for(auto vector2: ans)
    {
        cout<<"Mst :"<<h<<endl;
        for(auto edge: vector2)
        {
